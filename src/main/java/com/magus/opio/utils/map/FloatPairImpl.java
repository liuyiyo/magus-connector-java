...
public class FloatPairImpl implements Pair {
    private byte[] src;
    private byte valType;
    private Map<Double, Long> keyPos;

    public FloatPairImpl(byte[] src, byte keyType, byte valType, int startPos) throws OPException {
        this.src = src;
        ...
        while (offset != total) {
            switch (keyType) {
                case VtFloat:
                    //float直接转double会存在精度丢失的情况
                    key = Double.parseDouble(String.valueOf(Bytes.GetFloat32(Arrays.copyOfRange(src, offset, offset + 4))));
                    offset += 4;
                    break;
                case VtDouble:
                    key = Bytes.GetFloat64(Arrays.copyOfRange(src, offset, offset + 8));
                    offset += 8;
                    break;
                default:
                    break;
    ...
    public void setKeyPos(Map<Double, Long> keyPos) {
        this.keyPos = keyPos;
    }
    ...
    public boolean getBool(Object key) {
        ...
        if (src.length == 0 || keyPos.size() == 0)
            return false;
        long pos = keyPos.getOrDefault(Common.obj2Double(key), (long) -1);
        if (pos == -1)
            return false;
        if (src[(int) pos] > 0)
            return true;
    ...
    public String getString(Object key) {
        ...
        if (src.length == 0 || keyPos.size() == 0)
            return "";
        long posLen = keyPos.getOrDefault(Common.obj2Double(key), (long) -1);
        if (posLen == -1)
            return "";
